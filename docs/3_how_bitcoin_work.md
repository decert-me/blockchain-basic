...
在比特币的公共共享总帐本中，记录了所有地址持有的余额，比特币的[账户](https://learnblockchain.cn/tags/账户?map=EVM)是用地址来表示。 
...
实际上，在比特币账本中，并不是的记录某个账户的余额是多少（和[以太坊](https://learnblockchain.cn/tags/以太坊?map=EVM)的[账户](https://learnblockchain.cn/tags/账户?map=EVM)余额模型不一样），比特币引入了一个“未花费的交易输出”（UTXO： Unspent Transaction Output）概念，一个 UTXO 代表 “一整块” 的可以使用比特币。UTXO 作为交易的输入。
...
1. 交易输入： 他们指向之前的交易创建的 UTXO，通常[钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)会收集当前可用的 UTXO 集合作为交易的输入；
2. 交易输出：表明多少[比特币](https://learnblockchain.cn/tags/比特币?map=BTC)会锁定到哪些地址，即生成新的 UTXO 。
3. 签名： 用私钥来解锁交易输入的UTXO。



创建比特币交易是在[钱包](https://learnblockchain.cn/tags/%E9%92%B1%E5%8C%85)内完成的，而不是在节点上，因此可以在离线的情况下创建交易，交易创建后，通过比特币节点发送到[比特币](https://learnblockchain.cn/tags/比特币?map=BTC)网络中。 
